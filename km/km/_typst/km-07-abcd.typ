= ABCD Statemets
<abcd-statemets>
to be practical, knowledge shoud be in ABCD form of statements: Given
\[C\]onditions, \[A\]ctor \[B\]ehaves to a certain \[D\]egree. So
Percptions captures conditions and actors, understanding stores relevant
ABCD statesments, Decision making creatively arrange plan based on
cobinations of ABCD principles, Acting implement the plans by realizing
ABCD principles (in a transformational engine built using ABCD
principles), and Learning and evaluating adjust the ABCD princilples
accodig to reality or results for th next cycle

== Example of ABCD Statement
<example-of-abcd-statement>
Given \[C\]ondition: "the weather is rainy", \[A\]ctor: "a commuter",
\[B\]ehaves: "takes an umbrella", \[D\]egree: "always". This ABCD
statement can be represented as: Given the condition "the weather is
rainy", the actor "a commuter" behaves by "taking an umbrella" to the
degree of "always". \#\# Another Example of ABCD Statement Given
\[C\]ondition: "the traffic is heavy", \[A\]ctor: "a delivery driver",
\[B\]ehaves: "leaves earlier", \[D\]egree: "often". This ABCD statement
can be represented as: Given the condition "the traffic is heavy", the
actor "a delivery driver" behaves by ”

leaving earlier” to the degree of "often". \#\# Yet Another Example of
ABCD Statement Given \[C\]ondition: "the stock market is volatile",
\[A\]ctor: "an investor", \[B\]ehaves: "diversifies their portfolio", \
\[D\]egree: "frequently". This ABCD statement can be represented as:
Given the condition "the stock market is volatile", the actor "an
investor" behaves by ”

diversifying their portfolio” to the degree of "frequently". \
\#\# One More Example of ABCD Statement Given \[C\]ondition: "the
deadline is approaching", \[A\]ctor: "a student", \[B\]ehaves:
"prioritizes tasks", \[D\]egree: "usually".

This ABCD statement can be represented as: Given the condition "the
deadline is approaching", the actor "a student" behaves by "prior
itizing tasks" to the degree of "usually". \#\# Final Example of ABCD
Statement \
Given \[C\]ondition: "the power outage occurs", \[A\]ctor: "a
homeowner", \[B\]ehaves: "uses a generator", \[D\]egree: "sometimes".
This ABCD statement can be represented as: Given the condition "the
power outage occurs", the actor "a homeowner" behaves by "using a
generator" to the degree of "sometimes".
